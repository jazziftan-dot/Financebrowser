        ...
        <div style="min-width:0">
          ...
          <div style="margin-top:5px">
            ...
            <div style="font-size:11px;color:var(--text2);margin-top:3px">
              ${pct.toFixed(0)}% abgezahlt · <span class="payoff-chip">${months ? 'Frei in ' + formatETA(months) : 'Rate erhöhen!'}</span>
            </div>
          </div>
        </div>
      </div>
...
function renderFIRE() {
  ...
  el('fire-card').innerHTML = `
    ...
    <div style="font-size:12px;color:var(--text2);margin-bottom:14px">Finanzielle Freiheit bei ${state.settings.fireWithdrawalRate}% Entnahmerate</div>
    <div class="kpi-grid">
      <div class="kpi-item"><div class="kpi-label">FIRE-Zahl</div><div class="kpi-value">${fmtK(fn)}</div></div>
      <div class="kpi-item"><div class="kpi-label">Depotwert</div><div class="kpi-value green">${fmtK(state.portfolioValue)}</div></div>
      <div class="kpi-item"><div class="kpi-label">Ruhestand/Mt.</div><div class="kpi-value">${fmtK(fexp)}</div></div>
      <div class="kpi-item"><div class="kpi-label">Noch benötigt</div><div class="kpi-value red">${fmtK(remaining)}</div></div>
    </div>
    <div style="margin:14px 0 6px;display:flex;justify-content:space-between;font-size:13px">
      <span style="color:var(--text2)">Fortschritt</span><strong>${fp.toFixed(1)}%</strong>
    </div>
    ...
    <div style="margin-top:12px;padding:12px;background:var(--surface2);border-radius:var(--radius-sm);text-align:center;font-size:14px">
      ${feta ? `🎯 Finanzielle Freiheit in ca. <strong>${formatETA(feta)}</strong>`
             : fn > 0 ? `<span style="color:var(--text2)">Trage monatliche Investitionen ein</span>`
                      : `<span style="color:var(--text2)">Erfasse Einkommen und Ausgaben</span>`}
    </div>
...
function renderGoals() {
  ...
  if (!state.goals.length) { list.innerHTML = emptyState('🎯', 'Noch keine Sparziele festgelegt.'); return; }
  list.innerHTML = state.goals.map(g => {
    const rem = Math.max(0, g.targetAmount - g.currentAmount);
    const pct = g.targetAmount > 0 ? Math.min(100, g.currentAmount / g.targetAmount * 100) : 0;
    let etaLabel = '–', etaMonths = null;
    if (rem === 0) { etaLabel = 'Erreicht! 🎉'; }
    else if (sav > 0) {
    ...
    <div class="card goal-card-new">
      ...
      ${etaMonths ? `<div class="goal-eta-box">
      ...
      </div>` : sav <= 0 ? `<div class="goal-eta-box" style="border-color:var(--red)">
        <div style="font-size:13px;color:var(--red)">⚠️ Kein freies Kapital – überprüfe deine Ausgaben</div>
      </div>` : ''}
      ${insights.length ? `<div style="margin-top:10px">
